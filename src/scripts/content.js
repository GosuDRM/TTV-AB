// TTV AB v9.0.0 - Twitch Ad Blocker
...
(() => {
	...
	function _collapseOldestPendingCounterMessage() {
		for (let i = 0; i < _pendingBridgeMessages.length; i++) {
			...
			for (let j = _pendingBridgeMessages.length - 1; j > i; j--) {
				if (
					_getPendingBridgeCounterIdentity(_pendingBridgeMessages[j]) !==
					identity
				) {
					continue;
				}
	...
	function _$ds(scope) {
		scope.__TTVAB_STATE__ = {
			...
			RewriteNativePlaybackAccessToken:
				_$c.REWRITE_NATIVE_PLAYBACK_ACCESS_TOKEN ?? false,
			...
			AdEndNativeRecoveryProbeCooldownMs:
				_$c.AD_END_NATIVE_RECOVERY_PROBE_COOLDOWN_MS ?? 750,
		...
		if (liveSegments.length > 0 && info) {
			...
			if (!Number.isNaN(seq)) {
				info._RecoveryStartSeq =
					seq +
					Math.max(0, liveSegments.length - info._RecoverySegments.length);
			}
		}
			...
			for (const line of lines) {
				if (line.startsWith("#EXT-X-MEDIA-SEQUENCE:")) {
					if (info) {
						...
						const seq = baseSeq + info._EmptyPlaylistFillerCount * 2;
	...
	function _getResolvedSilentBackupHoldMaxMs() {
		return Math.max(
			0,
			Number(__TTVAB_STATE__?.SilentBackupHoldMaxMs) || 120000,
		);
	}
	...
	function _doesPlaybackContextMatchInfo(
		...
		return Boolean(
			infoChannel && targetChannel && infoChannel === targetChannel,
		);
	}
	...
	function _getPinnedBackupPlayerTypeForInfo(info) {
		...
		if (pinnedContext.MediaKey && infoContext.MediaKey) {
			return pinnedContext.MediaKey === infoContext.MediaKey
				? pinnedType
				: null;
		}
	...
	async function _isAdEndStable(info, realFetch, resolution = null) {
		...
		const minCleanPlaylists = isLowLatency
			? Math.max(6, baseMinCleanPlaylists * 2)
		...
		try {
			...
			for (const key in __TTVAB_STATE__.StreamInfosByUrl) {
				try {
					...
					if (_$dl) _$l("_$gsi: error comparing stream info URL", "debug");
				}
			}
		} catch {
			if (_$dl) _$l("_$gsi: error looking up stream info by URL", "debug");
		}
	...
	async function _canReloadNativePlayerAfterAd(
		...
		const probeCooldownMs = Math.max(
			500,
			Number(__TTVAB_STATE__?.AdEndNativeRecoveryProbeCooldownMs) || 1500,
		);
		const now = Date.now();
		if (
		...
		try {
			...
			const targetResolution =
				resolution || _$gfr(info, "") || info?.ResolutionList?.[0] || null;
	...
	async function _$pm(url, text, realFetch) {
		...
		const hasAdsSync =
			...
			_$pka(text, { includeCached: false });
		if (info._ProcessingM3U8) {
			if (hasAdsSync) {
				if (info.LastCleanBackupM3U8) {
					info._CachedServeCount = (info._CachedServeCount || 0) + 1;
					return _$im(info.LastCleanBackupM3U8, info._CachedServeCount * 2);
				}
				if (info.LastCleanNativeM3U8) {
					info._CachedServeCount = (info._CachedServeCount || 0) + 1;
					return _$im(info.LastCleanNativeM3U8, info._CachedServeCount * 2);
				}
				return _$sa(text, true, info, true, true);
			}
			return text;
		}
		info._ProcessingM3U8 = true;
		try {
			...
			if (hasAds) {
				...
				if (info.IsHoldingBackupAfterAd) {
					const holdElapsed =
						Date.now() -
						Math.max(0, Number(info.SilentBackupHoldStartedAt) || 0);
				...
				if (info.PendingAdEndAt || info.CleanPlaylistCount) {
					...
					if (
						lastAdEndBounceAt > 0 &&
						now - lastAdEndBounceAt < bounceDebounceMs
					) {
					...
					for (let j = 0; j < lines.length; j++) {
						...
						if (
							...
							try {
								...
								if (_$dl)
									_$l(
										"_$fb: error in fire-and-forget backup stream request",
										"debug",
									);
							}
							break;
				...
				if (info.IsUsingFallbackStream) {
					const stripped = _$sa(text, false, info, false, true);
					if (stripped !== text) {
						return stripped;
					}
					...
					const emptyUrl =
						...
						"";
					if (emptyUrl) {
						const seq = (info._FallbackFillerCount || 1) * 2;
						const fillerLines = [
							"#EXTM3U",
							"#EXT-X-VERSION:3",
				...
				if (info.CsaiOnlyThisBreak) {
					const stripped = _$sa(text, false, info);
				...
				if (
					...
					if (canServeStopgap) {
						...
						info._BackupSearchStartedAt = Date.now();
						_$fb(info, realFetch, startIdx, res)
							.then(() => {
								info._BackupSearchStartedAt = 0;
							})
							.catch(() => {
						...
						const strippedFallback = _$sa(text, false, info, true);
				...
				if (info._BackupSearchStartedAt) {
					if (isCsaiOnly) {
						const stripped = _$sa(text, false, info, true, true);
				...
				const shouldForceStrip = stripHevc || info.IsUsingFallbackStream;
				if (__TTVAB_STATE__.IsAdStrippingEnabled || shouldForceStrip) {
					text = _$sa(text, shouldForceStrip, info);
				}
	...
	async function _$fb(info, realFetch, startIdx = 0, currentResolution = null) {
		...
		if (info.NeedsAutoplayFallback && playerTypes.includes("autoplay")) {
			playerTypes = [
				"autoplay",
				...playerTypes.filter((t) => t !== "autoplay"),
			];
		}
		...
		for (let pi = 0; !backupM3u8 && pi < playerTypesLen; pi++) {
			...
			let encBaseUrl =
				typeof encCache === "object" && encCache?.baseUrl
					? encCache.baseUrl
					: info.UsherBaseUrl;
			if (!enc && !isFullyCachedPlayerType) {
				isFreshM3u8 = true;
				try {
					...
					if (tokenRes.status === 200) {
						...
						if (sig && tokenValue) {
							...
							const missingParts = [
								...
								.join("+");
							_$l(
								`Missing token ${missingParts || "parts"} for ${pt}`,
								"warning",
							);
							_markBackupPlayerRetryCooldown(info, pt, "token-error");
							continue;
					...
					_$l(`Backup error for ${pt}: ${_e.message}`, "error");
			...
			if (enc) {
				try {
						...
						if (streamRes.status === 200) {
							...
							if (m3u8) {
								...
								if (promotionPolicy.allowSelectedPromotion) {
									if (realPt === "autoplay") {
										info.NeedsAutoplayFallback = true;
									}
					...
					_$l(`Stream error for ${pt}: ${_e.message}`, "error");
					_markBackupPlayerRetryCooldown(info, pt, "stream-error");
					invalidateCache = true;
				}
			}
	...
	const _POST_AD_RESET_SELECTOR_GROUP =
		_POST_AD_RESET_ONLY_SELECTORS.join(", ");
	...
	function _$wf() {
		...
		const _emptySegmentBinary = atob(EMPTY_SEGMENT_B64);
		const _emptySegmentBytes = new Uint8Array(_emptySegmentBinary.length);
		for (let i = 0; i < _emptySegmentBinary.length; i++) {
			_emptySegmentBytes[i] = _emptySegmentBinary.charCodeAt(i);
		}
		const EMPTY_SEGMENT_URL = URL.createObjectURL(
			new Blob([_emptySegmentBytes], { type: "video/mp4" }),
		);
		if (typeof __TTVAB_STATE__ !== "undefined" && __TTVAB_STATE__) {
						...
						if (codecs.startsWith("hev") || codecs.startsWith("hvc")) {
							...
							const closest = [...nonHevcList].sort((a, b) => {
								...
								const aArea =
									...
									(Number.isFinite(ah) ? ah : 0);
								const bArea =
									(Number.isFinite(bw) ? bw : 0) *
									(Number.isFinite(bh) ? bh : 0);
								return (
			...
			try {
				...
				if (playbackContext?.MediaKey) {
					...
					try {
						...
						_$l(
							`Master playlist processing failed for ${logTarget}: ${err?.message ?? String(err)}`,
							"error",
						);
					...
					if (response.status === 200) {
						...
						try {
							...
							if (err?.name !== "AbortError") {
								_$l(
									`Media playlist processing failed for ${url}: ${err?.message ?? String(err)}`,
									"error",
								);
							}
							try {
	...
	function _$hw() {
		...
		const createHookedWorkerConstructor = (BaseWorker) => {
            ...
            `;
					...
					const isPlaybackContextMismatch = (
						expectedContext,
						currentContext,
					) => {
						const normalizedExpectedContext =
					...
					this.addEventListener("message", (e) => {
						...
						switch (data.key) {
							case "FetchRequest":
								void handleWorkerFetchRequest(data.value).then(
									(responseData) => {
										try {
							...
							case "AdDetected":
								...
								}
								{
									...
									} else if (isContinuation && typeof _$rpfa === "function") {
										const cooldownMs =
							...
							case "ReloadPlayer":
								...
								if (typeof _$dpt === "function") {
									...
									_$dpt(false, true, {
										...
										newMediaPlayerInstance:
											data.newMediaPlayerInstance !== false,
									});
								}
								break;
							default:
	...
	function _$mf() {
				...
				for (const op of operations) {
					...
					if (typeof op.variables.playerType === "string") {
						if (op.variables.playerType !== forceType) {
							previousPlayerType =
								previousPlayerType || op.variables.playerType;
							op.variables.playerType = forceType;
							changed = true;
						}
						const expectedPlatform =
							forceType === "autoplay" ? "android" : "web";
		...
		const processGqlBody = (bodyText) => {
			...
			try {
				...
				if (_$dl) _$l("processGqlBody: error in GQL body parsing", "debug");
			}
		};
		const processGqlResponse = async (response) => {
			...
			try {
				...
				if (_$dl) _$l("window.fetch hook: error in GQL interception", "debug");
			}
		};
	...
	function _hasRecentPlaybackControlInteraction(
		channel = null,
		mediaKey = null,
	) {
	...
	function _scheduleSecondaryPlayerHandoffPause(
		channel = null,
		mediaKey = null,
	) {
	...
	function _syncPrimaryMediaPlaybackIntent() {
		...
		const handlePause = () => {
			...
			const mediaKey = _resolvePlayerMediaKey(
				null,
				__TTVAB_STATE__.PageMediaKey,
			);
			if (!mediaKey) return;
		...
		if (!window.__TTVAB_WINDOW_OPEN_PATCHED__) {
			...
			try {
				window.open = function patchedWindowOpen(...args) {
					const descriptor = _getSecondaryPlayerLaunchDescriptorFromUrl(
						args[0],
					);
				...
				_beginSecondaryPlayerHandoff(
					{
						kind: "pip",
						channel:
							_normalizePlayerChannel(__TTVAB_STATE__.PageChannel) || null,
						mediaKey:
							_normalizeMediaKey(__TTVAB_STATE__.PageMediaKey) ||
							_resolvePlayerMediaKey(__TTVAB_STATE__.PageChannel, null),
					},
	...
	function _$rpa(channel = null, mediaKey = null) {
		...
		if (video?.ended) {
			_$l(
				"Player ended after ad; deferring recovery to buffer monitor",
				"info",
			);
			return false;
		}
	...
	function _handlePostAdGraceWatch(
		...
		const now = Date.now();
		if (_$pbs.postAdGraceUntil <= 0 || now > _$pbs.postAdGraceUntil) {
			if (_$pbs.postAdGraceUntil > 0) _resetPostAdGrace();
			return false;
		}
		...
		if (_isPlayerPaused(player, playerCore, liveVideo)) {
			_$pbs.postAdGraceLastCurrentTime = Number(liveVideo.currentTime) || 0;
			_$pbs.postAdGraceStallTicks = 0;
			return false;
		}
		...
		const isStalled =
			liveVideoWidth <= 0 ||
			_$pbs.postAdGraceStallTicks >= _POST_AD_GRACE_STALL_TICKS_REQUIRED;
		if (!isStalled) return false;
		if (
			now - _$pbs.postAdGracePauseResumeAt >=
		...
		if (_$pbs.lastFixTime > now - _POST_AD_RECOVERY_RELOAD_COOLDOWN_MS) {
			return false;
		}
	...
	function _handlePendingPostAdRecovery(
		...
		const advanced =
			!isCycleStart && liveCurrentTime > _$pbs.postAdLastCurrentTime + 0.05;
		...
		_$pbs.postAdUnhealthyCount++;
		if (
			_$pbs.postAdUnhealthyCount >= _POST_AD_UNHEALTHY_RELOAD_COUNT &&
			_$pbs.lastFixTime <= now - _POST_AD_RECOVERY_RELOAD_COOLDOWN_MS
		) {
			if (!canSoftReload) {
	...
	function _$dpt(isPausePlay, isReload, options = {}) {
		...
		if (isReload) {
			...
			const preferenceSnapshot = _$cps(
				playerCore,
			...
			if (_$cpr) {
				try {
					...
					if (!playerCore) {
						...
						!player.getHTMLVideoElement()?.ended &&
						_$pbs.lastFixTime <= Date.now() - _getLowLatencyMinRepeatDelay()
					) {
						const {
							video,
							position,
						...
						if (
							...
							_$pbs.numSame++;
							if (
								_$pbs.numSame === __TTVAB_STATE__.PlayerBufferingSameStateCount
							) {
								_$l(

...
function _checkPinnedBackupStall(player) {
    ...
    if (_PinnedBackupStallState.forceRefreshCount >= 3) {
        _$l(`Pinned backup stalled (${pinnedType}): currentTime=${currentTime.toFixed(2)}s, bufferEnd=${bufferedEnd.toFixed(2)}s, buffer not growing for ${Math.round((now - _PinnedBackupStallState.firstObservedAt) / 100) / 10}s — re-searches exhausted (${_PinnedBackupStallState.forceRefreshCount} attempts), leaving stream as-is`, "warning");
        return;
    }

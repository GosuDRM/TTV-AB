...
async function _getEmptyAdHoldResponse(url, realFetch, signal = null) {
	...
	const response = await realFetch(_EMPTY_HOLD_SEGMENT_URL, { signal });
	...
	const timestampOffset = BigInt(sequence) * 92160n;
	const timestampMask = 0x1ffffffffn;
	const advanceTimestamp = (offset) => {
		const timestamp =
			(BigInt(bytes[offset] & 14) << 29n) |
			(BigInt(bytes[offset + 1]) << 22n) |
			(BigInt(bytes[offset + 2] & 254) << 14n) |
			(BigInt(bytes[offset + 3]) << 7n) |
			(BigInt(bytes[offset + 4]) >> 1n);
		const advanced = (timestamp + timestampOffset) & timestampMask;
		bytes[offset] = (bytes[offset] & 241) | Number((advanced >> 29n) & 14n);
		bytes[offset + 1] = Number((advanced >> 22n) & 255n);
		bytes[offset + 2] = Number((advanced >> 14n) & 254n) | 1;
		bytes[offset + 3] = Number((advanced >> 7n) & 255n);
		bytes[offset + 4] = Number((advanced << 1n) & 254n) | 1;
	};
	let hasVideoTimestamp = false;
	let hasAudioTimestamp = false;
	for (let offset = 0; offset < bytes.length; offset += 188) {
		...
		if (bytes[offset + 3] & 32) {
			...
			if (adaptationLength >= 7 && bytes[payload + 1] & 16) {
				const pcr = payload + 2;
				const timestamp =
					(BigInt(bytes[pcr]) << 25n) |
					(BigInt(bytes[pcr + 1]) << 17n) |
					(BigInt(bytes[pcr + 2]) << 9n) |
					(BigInt(bytes[pcr + 3]) << 1n) |
					(BigInt(bytes[pcr + 4]) >> 7n);
				const advanced = (timestamp + timestampOffset) & timestampMask;
				bytes[pcr] = Number((advanced >> 25n) & 255n);
				bytes[pcr + 1] = Number((advanced >> 17n) & 255n);
				bytes[pcr + 2] = Number((advanced >> 9n) & 255n);
				bytes[pcr + 3] = Number((advanced >> 1n) & 255n);
				bytes[pcr + 4] = (bytes[pcr + 4] & 127) | Number((advanced & 1n) << 7n);
			}
		...
		const timestampFlags = bytes[payload + 7] >> 6;
		if (timestampFlags !== 2 && timestampFlags !== 3) continue;
		advanceTimestamp(payload + 9);
		if (timestampFlags === 3) {
		...
		hasVideoTimestamp ||= streamId >= 224 && streamId <= 239;
		hasAudioTimestamp ||= streamId >= 192 && streamId <= 223;
	}
...
function _isEmptyAdHoldSegmentUrl(url) {
	...
	try {
		...
		return (
			...
			parsed.pathname === "/__ttvab_empty_hold_segment.ts"
		);
	} catch {
		return false;
	}

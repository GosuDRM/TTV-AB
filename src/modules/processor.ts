...
function _resetStreamAdState(info, preserveEmptyHoldTimelines = false) {
	...
	info._LastNoBackupProbeAt = 0;
	info._NoBackupRecoveryCandidates?.clear?.();
...
function _getEarlyNoBackupRetry(info, startIdx = 0, codecs = null) {
	...
	const searchCompletedAt = Number(info?._LastBackupSearchCompletedAt) || 0;
	const regularRetryAt = searchCompletedAt + 15000;
	...
	if (
		...
		now - (Number(info._LastNoBackupProbeAt) || 0) < 1500 ||
		...
		_getVideoCodecFamily(codecs) !== "avc" ||
		info._BackupSearchPromise ||
		info._BackupSearchPromises?.size > 0 ||
		searchCompletedAt <= 0 ||
		searchCompletedAt < cycleStartedAt ||
		now < searchCompletedAt ||
		deadlineAt - now < 1000 ||
		_isRecentPostAdReentry(info) ||
		_getPendingForegroundQualityProbeAt(info) > 0 ||
		Number(__TTVAB_STATE__?.BackupSearchForceRefreshAt) > 0
	) {
		return null;
	}
	const candidates = _getOrderedBackupPlayerTypes(info, startIdx)
		.filter((type) => ["site", "embed", "popout", "mobile_web"].includes(type))
		.map((playerType) => ({
			playerType,
		...
		.filter(
			({ candidate }) =>
				candidate &&
				candidate.cycleStartedAt === cycleStartedAt &&
				candidate.backupSearchEpoch === backupSearchEpoch &&
				now - candidate.createdAt <= 60000 &&
				now - candidate.lastProbeAt >= 900,
		);
	for (const { candidate } of candidates) {
		if (now - candidate.cleanStartedAt > 5000) {
			candidate.cleanStartedAt = 0;
			candidate.cleanMediaSequence = null;
		}
	}
	candidates.sort(
		(a, b) =>
			Number(b.candidate.cleanStartedAt > 0) -
				Number(a.candidate.cleanStartedAt > 0) ||
			a.candidate.lastProbeAt - b.candidate.lastProbeAt,
	);
...
function _startEarlyNoBackupRetry(
	...
	void _findBackupStream(
		...
		(result) => {
			if (
				...
				_isBackupSearchContextCurrent(
					info,
					retry.backupSearchEpoch,
					retry.cycleStartedAt,
				)
			) {
				info._LastBackupSearchCompletedAt = 0;
			}
		...
		(error) => {
			_log(
				`[Recovery] Early backup retry failed: ${error?.message ?? String(error)}`,
				"warning",
			);
		},
	);
...
function _getNativeRecoveryMaster(info, forRefresh = false) {
	const current = {
		...
		resolutionList: Array.isArray(info?.ResolutionList)
			? info.ResolutionList
			: [],
	};
	...
	const requestedHeight =
		Number(String(quality || "").match(/^(\d+)p/)?.[1]) || 0;
	const currentTarget = _getResolutionByQualityGroup(
		current.resolutionList,
		quality,
	);
	const currentHeight =
		Number(String(currentTarget?.Resolution || "").split("x")[1]) || 0;
	if (
		!saved ||
		info.MediaType === "vod" ||
	...
	const availableHeight = current.resolutionList.reduce(
		(height, entry) =>
			Math.max(
				height,
...
function _resolveAdBackupTargetResolution(
	...
	const preferredResolution = _resolvePreferredBackupResolution({
		...info,
...
async function _isAdEndStable(
	...
	if (
		...
		const playlistUrl = _getExactPlaylistUrlKey(
			_getStreamUrl(
				recoveryMaster.master,
				targetResolution,
				recoveryMaster.masterUrl,
			),
		);
		const variant = recoveryMaster.resolutionList.find(
			(entry) => entry.Url === playlistUrl,
		);
		if (playlistUrl && variant && _getVideoCodecFamily(variant.Codecs)) {
...
function _alignLivePlaylist(info, text, backupMetadata = null) {
	...
	const minimumTime = changedSource
		? backupMetadata || previous?.backup || previous?.afterHold
			...
			: 0
		: previous.minimumTime;
	const lines = text.split(/\r?\n/);
	const entries = [];
	let nextTime = Number.NaN;
	...
	for (let index = 0; index < lines.length; index++) {
		...
		if (line === "#EXT-X-DISCONTINUITY") {
			discontinuity++;
			if (!hasExplicitTime) nextTime = Number.NaN;
		}
		if (line.startsWith("#EXT-X-PROGRAM-DATE-TIME:")) {
			nextTime = Date.parse(line.slice(25));
		...
		for (let tag = blockStart; tag < uriIndex; tag++) {
			if (lines[tag].startsWith("#EXT-X-BYTERANGE:")) {
				const match = lines[tag].match(/^#EXT-X-BYTERANGE:(\d+)(?:@(\d+))?$/);
				const length = Number(match?.[1]);
				const offset =
					match?.[2] != null
						? Number(match[2])
						: previousRange?.uri === lines[uriIndex]
							? previousRange.end
							: Number.NaN;
				range = {
					index: tag,
			...
			if (tag > index && lines[tag] === "#EXT-X-DISCONTINUITY") {
				discontinuity++;
				if (!hasExplicitTime) nextTime = Number.NaN;
			}
			if (tag > index && lines[tag].startsWith("#EXT-X-PROGRAM-DATE-TIME:")) {
				nextTime = Date.parse(lines[tag].slice(25));
				hasExplicitTime = true;
			}
		}
		entries.push({
	...
	if (
		entries.some(
			(entry, index) =>
				...
				(index > 0 && entry.time < entries[index - 1].time),
		)
	) {
		if (minimumTime && text.includes("#EXT-X-PROGRAM-DATE-TIME:"))
			throw new DOMException(
				"Live handoff requires unambiguous broadcast timestamps",
				"AbortError",
			);
		return text;
	}
	if (text.includes("#EXT-X-SKIP:")) {
		if (minimumTime)
			throw new DOMException(
				"Live handoff requires a full dated playlist",
				"AbortError",
			);
		return text;
	}
	...
	let output = text;
	if (retainedIndex > 0) {
		const retained = entries[retainedIndex];
		const firstSequence = _parsePlaylistFirstMediaSequence(text) ?? 0;
		const prefix = lines
			.slice(0, retained.start)
			.filter(
				(line) =>
					line.startsWith("#") &&
					!/^#EXT(?:INF:|-X-(?:PROGRAM-DATE-TIME:|BYTERANGE:|GAP(?:$|:)|DISCONTINUITY(?:$|:)|PART:|PRELOAD-HINT:|TWITCH-PREFETCH(?:-DISCONTINUITY)?[:]?))/.test(
						line,
					),
			);
		const tail = lines.slice(retained.start);
		const priorDiscontinuities = tail
			.slice(0, retained.uriIndex - retained.start)
			.filter((line) => line === "#EXT-X-DISCONTINUITY").length;
		_setPlaylistDiscontinuitySequence(
			prefix,
			retained.discontinuity - priorDiscontinuities,
		);
		const sequenceIndex = prefix.findIndex((line) =>
			line.startsWith("#EXT-X-MEDIA-SEQUENCE:"),
		);
		const sequenceLine = `#EXT-X-MEDIA-SEQUENCE:${firstSequence + retainedIndex}`;
		if (sequenceIndex < 0) prefix.splice(1, 0, sequenceLine);
		else prefix[sequenceIndex] = sequenceLine;
		if (retained.range) {
			if (
				!Number.isSafeInteger(retained.range.offset) ||
				retained.range.offset < 0 ||
				!Number.isSafeInteger(retained.range.length) ||
				retained.range.length <= 0
			) {
				throw new DOMException(
					"Live handoff requires an explicit byte range",
					"AbortError",
				);
			}
			tail[retained.range.index - retained.start] =
				`#EXT-X-BYTERANGE:${retained.range.length}@${retained.range.offset}`;
		}
		const dated = tail
			.slice(0, retained.uriIndex - retained.start)
			.some((line) => line.startsWith("#EXT-X-PROGRAM-DATE-TIME:"));
		if (!dated)
			tail.splice(
				tail.findIndex((line) => line.startsWith("#EXTINF:")),
				0,
...
function _applyPlaylistContinuity(
	...
	try {
		...
		if (
			...
			__TTVAB_STATE__?.IsAdStrippingEnabled === true &&
			(info.IsShowingAd || info.IsHoldingBackupAfterAd) &&
			backupMetadata?.playerType === "autoplay" &&
			...
			_log(
				"[Recovery] Autoplay bridge served; retaining native rebuild requirement",
				"info",
			);
		}
		const candidate = requestContext?.nativeSessionToConsume;
		const pending = info?._PendingPostAdNativeMaster;
		if (
			pending &&
			candidate?.session === pending &&
			candidate.playlist === text &&
			!backupMetadata &&
			!pending.consumed
		) {
			const selected = info.Urls?.[_getMediaPlaylistSessionKey(url)];
			if (selected) {
				pending.codec = _getVideoCodecIdentity(selected.Codecs);
				pending.resolution = selected.Resolution;
			}
...
function _applyBackupSpliceBridge(info, text, backupMetadata = null) {
	...
	const backupCodec =
		...
		_getVideoCodecFamily(
			metadata?.codecFamily || info.LastCleanBackupCodecFamily,
		) ||
		"?";
...
function _getMediaPlaylistSessionKey(url) {
	...
	try {
		...
		parsed.search = parsed.search
			.slice(1)
			.split("&")
			.filter((parameter) => !/^_HLS_(?:msn|part|skip)(?:=|$)/.test(parameter))
			.join("&");
		return parsed.href;
	} catch {
		return exactUrl;
...
function _getEmptyHoldUpstreamUrl(info, url) {
	...
	try {
		const parsed = new URL(url);
		const requestedSequence = parsed.searchParams.get("_HLS_msn");
		const nativeSequence = Number(requestedSequence) - timeline.mediaOffset;
		const canTranslate =
			timeline.kind === "native" &&
			/^\d+$/.test(requestedSequence || "") &&
			Number.isSafeInteger(nativeSequence) &&
			nativeSequence >= 0;
		parsed.search = parsed.search
			.slice(1)
			.split("&")
			.flatMap((parameter) => {
				if (/^_HLS_skip(?:=|$)/.test(parameter)) return [];
				if (/^_HLS_msn(?:=|$)/.test(parameter)) {
					return canTranslate ? [`_HLS_msn=${nativeSequence}`] : [];
				}
				if (/^_HLS_part(?:=|$)/.test(parameter) && !canTranslate) return [];
				return [parameter];
			})
			.join("&");
		return parsed.href;
...
function _applyEmptyHoldPlaylistContinuity(
	...
	if (
		!isHold &&
		((metadata && (metadata.ambiguous || !metadata.playlistUrl)) ||
			(!metadata &&
				info.IsUsingBackupStream &&
				text === info.LastCleanBackupM3U8))
	) {
		throw new DOMException(
			"Empty hold recovery requires exact backup ownership",
			"AbortError",
		);
	}
	const kind = isHold ? "hold" : metadata ? "backup" : "native";
	const identity = JSON.stringify(
		isHold
			? [kind, info.VisibleAdStartedAt || info._PageFallbackCycleStartedAt || 0]
			: metadata
				? [
						kind,
						metadata.playerType,
	...
	const boundaryScrolledOut =
		timeline.addBoundary && firstSequence > timeline.boundarySequence;
	_setPlaylistDiscontinuitySequence(
		lines,
		baseDiscontinuity +
	...
	if (mediaSequenceIndex < 0) lines.splice(1, 0, mediaSequenceLine);
	...
	if (timeline.addBoundary && !boundaryScrolledOut) {
		...
		if (boundaryIndex >= 0)
			lines.splice(boundaryIndex, 0, "#EXT-X-DISCONTINUITY");
	}
	const output = [];
	...
	for (const line of lines) {
		if (line.startsWith("#EXT-X-KEY:")) {
			...
			if (
				...
				implicitKey =
					attributes.METHOD !== "NONE" && !attributes.IV ? line : null;
				keySequence = null;
			}
		}
		if (
			timeline.mediaOffset &&
			implicitKey &&
			...
			output.push(
				`${implicitKey},IV=0x${sequence.toString(16).padStart(32, "0")}`,
			);
			keySequence = sequence;
			addedIv = true;
		}
		...
		if (kind === "native" && line.startsWith("#EXT-X-RENDITION-REPORT:")) {
			...
			try {
				...
				const reportedSequence = Number(attributes["LAST-MSN"]);
				if (
					reportTimeline?.kind === "native" &&
					Number.isSafeInteger(reportedSequence) &&
					reportedSequence >= 0
				) {
					outputLine = line.replace(
						/([:,])LAST-MSN=\d+/,
						`$1LAST-MSN=${reportedSequence + reportTimeline.mediaOffset}`,
					);
				}
			} catch {}
		}
	...
	if (addedIv) {
		...
		if (versionIndex < 0) output.splice(1, 0, "#EXT-X-VERSION:2");
		else if (Number(output[versionIndex].split(":")[1]) < 2)
	...
	info._EmptyHoldTimelineByUrl.set(key, timeline);
	while (info._EmptyHoldTimelineByUrl.size > 32) {
		info._EmptyHoldTimelineByUrl.delete(
			info._EmptyHoldTimelineByUrl.keys().next().value,
		);
	}
...
async function _canReloadNativePlayerAfterAd(
	...
	const probeInvalidated = () =>
		...
		(ownedNativeRecoveryTarget &&
			(!probeIsLive ||
				!probeStreamUrl ||
				_getNativeRecoveryMaster(info).master !==
					ownedNativeRecoveryTarget.master ||
				_getNativeRecoveryMaster(info).masterUrl !==
					ownedNativeRecoveryTarget.masterUrl ||
				...
				!_getNativeRecoveryMaster(info).resolutionList.some(
					(entry) => entry.Url === probeStreamUrl,
				) ||
				Math.max(0, Number(info.NativeRecoveryLoaderEpoch) || 0) !==
					probeLoaderEpoch ||
...
async function _processM3U8(
	...
	if (
		...
		const compatibleResult = requestWasAdMarked
...
async function _processM3U8Core(
	...
	if (
		...
		(_getExactPlaylistUrlKey(pendingPostAdNativeMaster.playlistUrl) ===
			exactRequestUrl ||
			pendingPostAdNativeMaster.verifiedPlaylistUrls?.includes(
				exactRequestUrl,
			)) &&
		(hasAds ||
			(hasMediaSegments &&
		...
		if (hasAds) {
			...
			info._PendingPostAdNativeMaster = null;
		} else if (requestAdContext && !pendingPostAdNativeMaster.consumed) {
			requestAdContext.nativeSessionToConsume = {
	...
	if (!hasAds && hasMediaSegments && !info.IsShowingAd) {
		if (!info.IsHoldingBackupAfterAd) {
			...
			if (
				...
				!isBackupUrl &&
				!info.IsUsingBackupStream &&
				directResolution?.Url === _getMediaPlaylistSessionKey(url) &&
				directResolution.Resolution ===
					info.SustainedNativeResolution?.Resolution &&
				info.EncodingsM3U8 &&
				info.UsherBaseUrl &&
				...
				const pendingPostAdNativeMaster =
					...
					exactRequestUrl
						? {
								...
								codec:
									verifiedNativeRecoveryTarget?.codec ||
									info.Urls?.[
										verifiedNativeRecoveryTarget?.playlistUrl || exactRequestUrl
									]?.Codecs ||
									null,
								resolution:
									verifiedNativeRecoveryTarget?.resolution ||
									info.Urls?.[
										verifiedNativeRecoveryTarget?.playlistUrl || exactRequestUrl
									]?.Resolution ||
									null,
	...
	if (hasAds) {
		...
		const earlyBackupRetry = _getEarlyNoBackupRetry(
			info,
			startIdx,
			directResolution?.Codecs || res?.Codecs || null,
		);
		if (earlyBackupRetry) {
			_startEarlyNoBackupRetry(
				info,
				realFetch,
				startIdx,
				_resolveAdBackupTargetResolution(info, url) || res,
...
async function _findBackupStream(
	...
	let earlyRetryController = null;
	let earlyRetryTimeoutId = null;
	...
	const searchPromise = (async () => {
		try {
			...
			if (earlyRetryController) {
				...
				cycleSignal?.removeEventListener?.("abort", abortEarlyRetry);
				earlyRetryController.abort();
			}
...
async function _searchBackupStream(
	...
	const cycleStartedAt = Math.max(0, Number(info?.VisibleAdStartedAt) || 0);
	const requestSignal =
		earlyRetry?.signal || info?._AdCycleRequestController?.signal || null;
		...
		(!earlyRetry ||
			(__TTVAB_STATE__?.DisableAutoplayBackup === true &&
				...
				_normalizeMediaKey(__TTVAB_STATE__?.PageMediaKey) ===
					earlyRetry.mediaKey)) &&
		_isBackupSearchContextCurrent(info, backupSearchEpoch, cycleStartedAt);
	const markRetryCooldown = (playerType, reason) => {
		if (!earlyRetry) _markBackupPlayerRetryCooldown(info, playerType, reason);
	};
	if (!searchIsCurrent() || searchDeadlineExceeded()) {
		return { type: null, m3u8: null };
	}
	if (!earlyRetry) _forceClearBackupCooldownsIfStale(info);
	const previousPlaylistUrl = earlyRetry?.candidate.playlistUrl || null;
	const previousCleanAt = earlyRetry?.candidate.cleanStartedAt || 0;
	const previousCleanSequence =
		earlyRetry?.candidate.cleanMediaSequence ?? null;
	if (earlyRetry) {
		earlyRetry.candidate.cleanStartedAt = 0;
		earlyRetry.candidate.cleanMediaSequence = null;
	}

	let playerTypes = _getOrderedBackupPlayerTypes(info, startIdx);
	if (earlyRetry) {
		playerTypes = playerTypes.filter((type) => type === earlyRetry.playerType);
	}
	const foregroundQualityProbeAt = _getPendingForegroundQualityProbeAt(info);
				...
				if (enc) {
					...
					try {
						...
						if (streamUrl) {
							...
							if (earlyRetry && streamUrl !== previousPlaylistUrl) {
								earlyRetry.candidate.playlistUrl = streamUrl;
								_log(
									`[Recovery] Backup ${pt} target changed to ${selectedResolution}; restarting clean confirmation`,
									"info",
								);
							}
							...
							if (streamRes.status === 200) {
								...
								if (m3u8) {
									...
									if (earlyRetry && promotionPolicy.allowSelectedPromotion) {
										const mediaSequence =
											_parsePlaylistFirstMediaSequence(m3u8);
										if (mediaSequence === null) break;
										const cleanAge = Date.now() - previousCleanAt;
										if (
											streamUrl !== previousPlaylistUrl ||
											previousCleanAt <= 0 ||
											previousCleanSequence === null ||
											cleanAge > 5000 ||
											mediaSequence < previousCleanSequence
										) {
											earlyRetry.candidate.cleanStartedAt = Date.now();
											earlyRetry.candidate.cleanMediaSequence = mediaSequence;
											break;
										}
										if (
											cleanAge < 900 ||
											...
											earlyRetry.candidate.cleanStartedAt = previousCleanAt;
											earlyRetry.candidate.cleanMediaSequence =
												previousCleanSequence;
											break;
										}
									}
									if (
										...
										info.MediaType === "live" &&
										cycleStartedAt > 0 &&
										["site", "embed", "popout", "mobile_web"].includes(pt) &&
										!isSessionNeutralCandidate &&
										!info.IsUsingModifiedM3U8 &&
										!info.EnhancedDecoderCodecFamily &&
										!info.EnhancedDecoderCodec &&
										selectedCodecFamily === "avc" &&
										!info.LastCleanBackupM3U8 &&
										!info.IsUsingBackupStream &&
										!(Number(info.LastCleanBackupAt) >= cycleStartedAt)
									) {
										info._NoBackupRecoveryCandidates?.set?.(pt, {
									...
									markRetryCooldown(pt, promotionPolicy.reason);
								...
								if (earlyRetry) {
									info._NoBackupRecoveryCandidates.delete(pt);
									return { type: null, m3u8: null };
								}
								_log(`Stream failed for ${pt}: ${streamRes.status}`, "warning");
								markRetryCooldown(pt, "stream-error");
								if (isExactEnhancedPass) {
									failedExactCodecPlayerTypes.add(pt);
								}
								invalidateCache = true;
							...
							markRetryCooldown(pt, "no-stream-url");
							if (isExactEnhancedPass) {
								failedExactCodecPlayerTypes.add(pt);
							}
							invalidateCache = true;
	...
	if (backupM3u8) {
		if (earlyRetry) {
			info.BackupEncodingsM3U8Cache[backupType] = earlyRetry.candidate.cache;
		}
		info._NoBackupRecoveryCandidates?.clear?.();

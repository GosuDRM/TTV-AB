...
function _getForcedAdEndReentryWindowMs() {
	return Math.max(0, Number(_C?.FORCED_AD_END_REENTRY_WINDOW_MS) || 0);
}
...
async function _isAdEndStable(info, realFetch, resolution = null) {
	...
	if (maxWaitMs > 0 && now - info.PendingAdEndAt >= maxWaitMs) {
		if (
			...
			_log(
				"[Trace] Native recovery not ready after ad-end wait budget; holding backup stream",
				"warning",
			);
			return "wait";
		}
		...
		_resetNativeRecoveryReadyState(info);
		_log(
			"[Trace] Forcing native recovery reload after ad-end wait budget",
			"warning",
		);
		return "reload";
	}
...
async function _getBackupStreamForAdEndWait(info, realFetch, resolution = null) {
	if (
		!info?.IsUsingBackupStream &&
		!info?.IsUsingFallbackStream &&
		!info?.LastCleanBackupM3U8
	...
	let backupM3u8 = null;
	let isFallback = Boolean(info.IsUsingFallbackStream);
	...
	try {
		...
		if (refreshedBackup?.m3u8) {
			backupType = refreshedBackup.type || backupType;
			backupM3u8 = refreshedBackup.m3u8;
			isFallback = Boolean(refreshedBackup.isFallback);
		}
	} catch (err) {
		_log(
			`[Trace] Backup refresh during ad-end wait failed: ${err.message}`,
			"warning",
		);
	}

	if (!backupM3u8 && info.LastCleanBackupM3U8) {
		backupM3u8 = info.LastCleanBackupM3U8;
	}
	if (!backupM3u8) return null;
	...
	info.ActiveBackupResolution =
		resolution?.Resolution || info.ActiveBackupResolution || null;
	if (backupType) {
	...
	if (backupType && info.ActiveBackupPlayerType !== backupType) {
		info.ActiveBackupPlayerType = backupType;
		_log(`Holding backup during ad-end wait: ${backupType}`, "info");
	...
	if (__TTVAB_STATE__.IsAdStrippingEnabled || isFallback) {
		return _stripAds(backupM3u8, false, info);
	}
	return backupM3u8;
}
...
function _getStreamInfoForPlaylist(url) {
	...
	const pageMediaKey = __TTVAB_STATE__.PageMediaKey || null;
	...
	try {
		...
		for (const key in __TTVAB_STATE__.StreamInfosByUrl) {
			try {
				...
				if (pageMediaKey && info?.MediaKey !== pageMediaKey) {
					continue;
				}
...
async function _processM3U8(url, text, realFetch) {
	...
	if (!info) {
		...
		const hasKnownSegs = _playlistHasKnownAdSegments(text, {
			includeCached: false,
		});
		if (
			!hasMarkers &&
		...
		_log(
			`[Diag] No stream info but ad markers found (markers=${hasMarkers} segs=${hasKnownSegs}): ${url.slice(0, 100)}`,
			"warning",
		);
	...
	if (!isBackupUrl) {
		const isTrackedNativeUrl = currentAliases.some(
			(alias) =>
				info.Urls?.[alias] || __TTVAB_STATE__.StreamInfosByUrl?.[alias] === info,
		);
		if (!isTrackedNativeUrl) {
			const hasAdMarkers = _hasPlaylistAdMarkers(text);
			if (!hasAdMarkers) {
				isBackupUrl = true;
			}
		}
	...
	if (hasAds) {
		...
		if (adEndState === "wait") {
			const backupText = await _getBackupStreamForAdEndWait(
				info,
				realFetch,
				res,
			);
...
async function _findBackupStream(
	...
	const { selected, fallbacks } = await new Promise<{
		selected: any;
		fallbacks: any[];
	}>((resolve) => {
		let settled = 0;

...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadStatistics() {
        chrome.storage.local.get(["ttvStats", "ttvAdsBlocked"], (result) => {
            ...
            const achievements = Array.isArray(stats.achievements)
                ? [
                    ...new Set(stats.achievements.filter((id) => typeof id === "string" && ACHIEVEMENT_IDS.has(id))),
                ]
                : [];
    ...
    chrome.storage.local.get([
        ...
        if (chrome.runtime.lastError) {
            console.error("[TTV AB] Popup init read error:", chrome.runtime.lastError.message);
        }
        const safeResult = (result || {});
    ...
    function syncSubTogglesState(adblockEnabled) {
        if (adSpoofingToggle) {
            ...
            if (slider)
                slider.style.opacity = adblockEnabled ? "1" : "0.4";
        }
    ...
    toggle.addEventListener("change", () => {
        ...
        chrome.storage.local.set({ ttvAdblockEnabled: enabled }, () => {
            ...
            if (chrome.runtime.lastError) {
                console.error("[TTV AB] Popup toggle write error:", chrome.runtime.lastError.message);
    ...
    adSpoofingToggle.addEventListener("change", () => {
        ...
        chrome.storage.local.set({ ttvAdSpoofingEnabled: enabled }, () => {
            ...
            if (chrome.runtime.lastError) {
                console.error("[TTV AB] Popup ad spoofing toggle write error:", chrome.runtime.lastError.message);
    ...
    if (autoplayBackupToggle) {
        autoplayBackupToggle.addEventListener("change", () => {
            ...
            chrome.storage.local.set({ ttvAutoplayBackupEnabled: enabled }, () => {
                ...
                if (chrome.runtime.lastError) {
                    console.error("[TTV AB] Popup autoplay backup toggle write error:", chrome.runtime.lastError.message);
